    ...
    <div class="my-10 mx-96 w-full max-w-md bg-white rounded-lg shadow-xl p-8">
        ...
        <form action="{{ route('user.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="npm" id="npm" placeholder="Masukkan NPM" value="{{ old('nama', $user->nama) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300" />
                @foreach($errors->get('nama') as $error)
            ...
            <div>
                ...
                @endif
                <input type="file" name="foto" id="foto" value="{{ old('nama', $user->nama) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300" />
                @foreach($errors->get('nama') as $error)
                    <p class="mt-1 text-base text-white bg-gradient-to-r from-red-500 to-red-700 border border-red-600 rounded-xl p-3 shadow-md font-semibold">{{ $error }}</p>
                @endforeach
            </div>
